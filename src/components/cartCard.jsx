...
function CartCard({ isOpen, setIsOpen }) {
  ...
  const [numberOfProducts, setNumberOfProducts] = useState(
    Array(existingCartData.length).fill(1)
  );
  ...
  existingCartData.forEach((item) => {
    if (
      ...
      // Handle the case where price is undefined or not in the expected format
      console.error(`Invalid price format for item:`, item);
    }
  });
  // console.log(dataa); // array of price
  // console.log(numberOfProducts); //arryof frequency
      ...
      <div className="max-w-7xl w-full p-10">
                ...
                `}>
                  ...
                  <td className="py-6 px-4">
                    <div>
                      <input
                        type="number"
                        className="opacity-50 border-[0.5px] w-1/2 border-sky-500 font-thin text-center"
                        value={
                          numberOfProducts[index] <= 0
                            ? 1
        ...
        <div className=" max-w-xl ml-auto mt-6">
          <ul className="text-[#333] divide-y">
            ...
            <li className="flex flex-wrap gap-4 text-md py-3">
              Shipping <span className="text-slate-400">(2%)</span>{" "}
              <span className="ml-auto font-bold">${total * 0.02}</span>
            </li>
            <li className="flex flex-wrap gap-4 text-md py-3">
              Tax <span className="text-slate-400">(3%)</span>{" "}
              <span className="ml-auto font-bold">${total * 0.03}</span>
            </li>
            <li className="flex flex-wrap gap-4 text-md py-3 font-bold">
              Total{" "}
              <span className="ml-auto">
                ${total + total * 0.02 + total * 0.03}
              </span>
            </li>
          </ul>
          <button
